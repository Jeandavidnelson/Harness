...
def context_payload(context: CompactTaskContext) -> dict[str, object]:
    payload: dict[str, object] = {
        "observed_code": [
            {"source": edge.source, "target": edge.target, "relation": edge.relation, "provenance": edge.provenance}
            for edge in context.observed_edges
        ],
        "declared_context": [
            {"source": edge.source, "target": edge.target, "relation": edge.relation, "provenance": edge.provenance, "source_file": edge.source_file}
...
def capabilities_payload() -> dict[str, object]:
    return {
        ...
        "provenance": ["EXTRACTED", "INFERRED", "AMBIGUOUS", "DECLARED_CONTEXT"],
