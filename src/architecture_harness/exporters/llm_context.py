...
def render_llm_context(context: CompactTaskContext) -> str:
    ...
    lines.extend(f"{e.source} -> {e.target} [{e.evidence_origin.value}; confidence={e.provenance}]" for e in context.observed_edges)
    lines.extend(["", "Declared runtime:"])
    lines.extend(f"{e.source} -> {e.target} [{e.evidence_origin.value}; confidence={e.provenance}; source={e.source_file}]" for e in context.context_edges)
